...
## Usuari
Staff del coworking, o tu com a operador de la plataforma. **Tres rols**, no un de sol:

| Rol | Àmbit | Permisos |
|---|---|---|
| `tecnic` | Tota la plataforma (`tenant_id` = NULL) | Control total sobre tots els tenants |
| `tenant_admin` | Un tenant concret | Control total dins del seu tenant (equivalent al que abans anomenàvem simplement "staff") |
| `user` | Un tenant concret | Només els mòduls que se li concedeixin explícitament (camp `permisos`) |

| Camp | Tipus | Notes |
|---|---|---|
| id | UUID | |
| tenant_id | FK → Tenant | NULL si `rol = tecnic` |
| nom | text | |
| email | text | Login |
| rol | enum | `tecnic` / `tenant_admin` / `user` |
| permisos | jsonb | Array de mòduls concedits (només rellevant si `rol = user`), p. ex. `["reserves","clients"]` |

Contrasenya gestionada per Supabase Auth (`auth.users`), no com a camp propi.
...
**4. Control d'accés per rols (implementat amb Row Level Security)**
Cada mòdul (Recursos, Clients, Reserves/Ocurrències, Factures) comprova, a nivell de base de dades:
- Si ets `tecnic` → accés total, a qualsevol tenant.
- Si ets `tenant_admin` o `user` amb el mòdul concedit a `permisos`, **i** la fila pertany al teu tenant → accés.
- En qualsevol altre cas → denegat.

Això vol dir que encara que hi hagués un error al codi de l'aplicació (per exemple, oblidar filtrar per tenant en una consulta), la base de dades mateixa bloquejaria l'accés indegut. Ja està implementat a Supabase amb polítiques RLS i funcions auxiliars (`is_tecnic()`, `is_tenant_admin()`, `has_permission(mòdul)`).

**5. Relació Reserva → Ocurrència**
