...
type Textos = {
  zonaPerillosa: string;
  avisEliminarPermanent: string;
  confirma: string;
  cancela: string;
...
export function DeleteClientForm({
  ...
  return (
    <div className="mt-8 rounded-xl border border-red-200 bg-red-50 p-5 dark:border-red-900/40 dark:bg-red-950/20">
      <h2 className="text-sm font-semibold text-red-700 dark:text-red-400">{textos.zonaPerillosa}</h2>
      <p className="mt-1 text-sm text-red-600/80 dark:text-red-400/70">{textos.avisEliminarPermanent}</p>
      <div className="mt-4">
        <DeleteClientInline action={action} textos={textos} />
      </div>
    </div>
  );
}
