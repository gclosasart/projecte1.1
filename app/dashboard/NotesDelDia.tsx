    ...
    <div className="min-w-0 flex flex-col gap-3">
          ...
          {notes.map((n) => (
            <li key={n.id} className="group flex h-9 min-w-0 shrink-0 items-center gap-2.5">
              ...
              <span
                ...
                className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-teal-600 text-xs font-semibold text-white dark:bg-teal-500"
              >
      ...
      <form onSubmit={handleSubmit} className="mt-1 flex h-9 gap-2">
        <input
          ...
          className="h-9 min-w-0 flex-1 rounded-lg border border-black/10 bg-white px-3 text-sm leading-9 text-zinc-950 outline-none focus:border-teal-600 dark:border-white/10 dark:bg-zinc-900 dark:text-zinc-50"
        />
        <button
          type="submit"
          disabled={pending || !text.trim()}
          className="h-9 shrink-0 rounded-lg bg-teal-600 px-3 text-sm font-medium leading-9 text-white transition-colors hover:bg-teal-700 disabled:opacity-40"
        >
          {textos.afegeix}
        </button>
      </form>
