...
export function ForecastChart({ dies, textos }: { dies: DiaForecast[]; textos: TextosForecast }) {
  const maxVal = Math.max(1, ...dies.map((d) => d.sessions));
  const [animat, setAnimat] = useState(false);
  const capActivitat = dies.every((d) => d.sessions === 0);
        ...
        {dies.map((d, i) => (
          <div key={d.label} className="flex flex-1 flex-col items-center gap-1.5">
            <div className="flex h-32 items-end">
              <div
                className="group/bar relative w-6 rounded-t bg-teal-600 transition-[height] duration-700 ease-out"
                style={{
