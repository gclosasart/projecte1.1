      ...
      <div className="overflow-x-auto">
        <div className="grid min-w-[840px] grid-cols-7 overflow-hidden rounded-xl border border-black/10 dark:border-white/10">
          {dict.calendari.dies.map((nom) => (
            <div
              key={nom}
              className="border-b border-black/10 bg-teal-600 px-2 py-2 text-center text-xs font-semibold text-white dark:border-white/10 dark:bg-teal-500 dark:text-white"
            >
              {nom}
            </div>
          ))}
          ...
          {dies.map((d) => {
            ...
            return (
              <button
                ...
                className={`min-h-28 border-b border-r border-black/10 p-1.5 text-left last:border-r-0 hover:ring-1 hover:ring-inset hover:ring-teal-600 dark:border-white/10 dark:hover:ring-teal-500 ${
                  d.dinsDelMes ? "bg-white dark:bg-zinc-950" : "bg-teal-50 dark:bg-zinc-900/40"
                }`}
              >
                <span
                  className={`inline-flex h-5 w-5 items-center justify-center rounded-full text-xs ${
                    d.esAvui
                      ? "bg-teal-600 font-semibold text-white dark:bg-teal-500 dark:text-white"
                      : d.dinsDelMes
                ...
                {seleccionat.ocurrencies.map((oc) => (
                  <li key={oc.id}>
                    <Link
                      ...
                      onClick={() => setSeleccionat(null)}
                      className="block rounded-lg border border-black/10 p-3 transition-colors hover:border-teal-600 dark:border-white/10 dark:hover:border-teal-500"
                    >
